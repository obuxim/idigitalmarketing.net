...
export const AuthProvider = ({ children }) => {
  ...
  async function userValidation() {
    try {
      ...
      const { data } = await axios.post(`${authKey}/validate`, {}, config);
      dispatch({
  ...
  async function userLogOut() {
    try {
      ...
      const { data } = await axios.post(`${authKey}/revoke`, {}, config);
      dispatch({
  ...
  async function userRegistration(username, email, password) {
    try {
      ...
      await axios
        .post(
          "https://financelms.wpengine.com/wp-json/wp/v2/users/register",
